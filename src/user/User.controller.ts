...
export const login = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (user.role !== Role.ADMIN && user.role !== Role.SUBADMIN) {
    throw new ApiError(HTTP_STATUS.FORBIDDEN, "Access denied: Only admins and subadmins can log in");
  }
...
export const refreshAccessToken = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!incomingRefreshToken) {
      throw new ApiError(HTTP_STATUS.UNAUTHORIZED, "Unauthorized request: Refresh token not provided");
    }

    const refreshSecret = process.env.REFRESH_TOKEN_SECRET;
    if (!refreshSecret) {
      throw new ApiError(HTTP_STATUS.INTERNAL_SERVER_ERROR, "Server configuration error: Refresh secret is missing");
    }
...
export const updateUser = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!name && !phone && !avatar) {
      throw new ApiError(HTTP_STATUS.BAD_REQUEST, "At least one field (name, phone, avatar) is required to update");
    }
