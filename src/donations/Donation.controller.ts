...
export const createDonation = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (existingDonation) {
    throw new ApiError(HTTP_STATUS.CONFLICT, "Donation with this transaction ID already registered");
  }
...
export const getDonationsList = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const sanitizedDonations = donations.map((donation) => {
    const isOwnerOrStaff =
      ngoProfile && (donation as any).campaign.ngoId === ngoProfile.id;
...
export const getDonationDetails = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (donation.anonymous && donation.userId !== userId && !isNGOStaff && (userRole !== Role.ADMIN || isNGOStaff)) {
...
export const updateDonationStatus = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!isAuthorized) {
    throw new ApiError(HTTP_STATUS.FORBIDDEN, "Access denied: You cannot update donation status for this NGO");
  }
  ...
  return res
    .status(HTTP_STATUS.OK)
    .json(new ApiResponse(HTTP_STATUS.OK, "Donation status updated successfully", updatedDonation));
});

/**
 * 5. Donation Analytics Summary (NGO / Personal / Global scopes)
 */
export const getDonationAnalytics = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  const userRole = req.user?.role;
